...
function(add_plugin target)
    cmake_parse_arguments(ARG "STATIC;SHARED" "CLASS_NAME" "" ${ARGN})

    if (ARG_SHARED)
        if (NOT ARG_STATIC)
            set(plugin_type SHARED)
        else ()
            message(FATAL_ERROR "The STATIC and SHARED options were specified.")
        endif()
    else ()
    ...
    if (NOT TARGET ${target})    
        add_library(${target} ${plugin_type})
    else ()
    ...
    if (plugin_type STREQUAL "STATIC")
        target_compile_definitions(${target} PRIVATE QT_STATICPLUGIN)
    endif()

    if (ARG_CLASS_NAME)
        set_target_properties(${target} PROPERTIES QT_PLUGIN_CLASS_NAME ${ARG_CLASS_NAME})
    else ()
        set_target_properties(${target} PROPERTIES QT_PLUGIN_CLASS_NAME ${target})
    endif ()
endfunction()
