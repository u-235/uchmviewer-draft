...
#include <browser/browser-page.hpp> // BrowserPage
...
BrowserPage* ViewWindowMgr::current()
{
...
BrowserPage* ViewWindowMgr::addNewTab(bool set_active)
{
	BrowserPage* page = 0;
	QWidget* view = page->pageView();
	...
	connect(view, SIGNAL(dataLoaded(ViewWindow*)), this, SLOT(onWindowContentChanged(ViewWindow*)));
...
void ViewWindowMgr::setTabName(BrowserPage* page)
{
	TabData* tab = findTab(page);
	...
	if (tab)
	{
		...
		m_tabWidget->setTabText(m_tabWidget->indexOf(page->pageView()), title);
...
void ViewWindowMgr::restoreSettings(const Settings::viewindow_saved_settings_t& settings)
{
	...
	{
		BrowserPage* page = addNewTab(false);
		page->load(settings[i].url);   // will call setTabName()
		page->setScrollTop(settings[i].scroll_y);
		page->setZoomFactor(settings[i].zoom);
	}
}
...
void ViewWindowMgr::saveSettings(Settings::viewindow_saved_settings_t& settings)
{
	...
	for (int i = 0; i < m_tabWidget->count(); i++)
	{
		...
		settings.push_back(Settings::SavedViewWindow(tab->page->url().toString(),
		                                             tab->page->scrollTop(),
		                                             tab->page->zoomFactor()));
	}
}
...
void ViewWindowMgr::onTabChanged(int newtabIndex)
{
	...
	{
		emit updateHistory();
		mainWindow->browserChanged(tab->page);
		tab->widget->setFocus();
	}
}
...
ViewWindowMgr::TabData* ViewWindowMgr::findTab(BrowserPage* page)
{
	for (WindowsIterator it = m_Windows.begin(); it != m_Windows.end(); ++it)
		if (it->page == page)
			return (it.operator->());
...
void ViewWindowMgr::findText(bool backward)
{
	BrowserPage::FindFlags flags;
	flags.backward = backward;
	flags.caseSensitively = checkCase->isChecked();
	flags.highlight = pConfig->browser.highlightSearchResults;
	current()->findText(editFind->text(), flags);
}

void ViewWindowMgr::onFindTextFinished(bool found)
{
...
void ViewWindowMgr::onWindowContentChanged(BrowserPage* page)
{
	setTabName(page);
}
