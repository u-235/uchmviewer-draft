...
ViewWindowMgr::ViewWindowMgr( PluginManager* pluginManager, QWidget* parent )
	: QWidget( parent ),
	  Ui::TabbedBrowser(),
...
UBrowser::Browser* ViewWindowMgr::addNewTab( EBook::Ptr ebook, bool set_active )
{
	UBrowser::Builder* builder = m_pluginManager->browserBuilder( ebook->kind() );
...
void ViewWindowMgr::restoreSettings( EBook::Ptr ebook, const Settings::viewindow_saved_settings_t& settings )
{
	...
	{
		UBrowser::Browser* browser = addNewTab( ebook, false );
