...
#include <browser/content-provider.hpp> // ContentProvider, ContentProviderPtr
...
/**
 * @brief The BrowserController class provides an interface to display documents.
 *
 * The application uses the BrowserController class to display eBook pages.
 * Hereinafter the word "controller" is used to mean "the BrowserController
 ...
 *
 * The application passes a pointer to a ContentProvider instance to the
 * controller constructor. The ContentProvider class is the only way to get
 * content to display the page. The controller should not attempt to load
 * content from the network or any other way other than from the ContentProvider.
 * The application starts loading the page by calling the load() or reload()
 * method. The loading can also be triggered by the management methods of the
 * navigation history, discussed @ref browser-feature-history "below". When the
 * load is complete, the controller emits the loadFinished() signal.
 *
...
class BrowserController : public QObject
{
	...
	protected:
		...
		ContentProviderPtr m_contentProvider;
	...
	public:
		 ...
		 */
		virtual void configure(const BrowserSettings& settings) = 0;
		 ...
		 * return @c nullptr.
		 *
		 * @return Returns a pointer to the BrowserHistory object or @c nullptr.
		 */
		virtual BrowserHistory* history() const = 0;
	...
	public:
		 ...
		 * @see selectAll()
		 */
		virtual void selectedCopy() const = 0;
		 ...
		 *     result.
		 */
		virtual void findText(const QString& text, const FindFlags& flags) = 0;
