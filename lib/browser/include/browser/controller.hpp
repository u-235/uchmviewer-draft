...
#ifndef BROWSER_CONTROLLER_HPP
#define BROWSER_CONTROLLER_HPP
...
#include <browser/types.hpp>            // for Feature, OpenFlag, Option
...
/**
 * @brief The Browser::Controller class provides an interface to display documents.
 *
 * The application uses the Browser::Controller class to display eBook pages.
 * Hereinafter the word "controller" is used to mean "the Browser::Controller
 * class or its descendant". Depending on the implementation, there may be stubs
 ...
 * inform the application of supported features. The controller may not have all
 * settings options via the configure() method, and the hasOption() method should
 * inform the application of supported options.
 *
 * The application passes a pointer to a Browser::ContentProvider instance to the
 * controller constructor. The Browser::ContentProvider class is the only way to
 * get content to display the page. The controller should not attempt to load
 * content from the network or any other way other than from the
 * Browser::ContentProvider. The application starts loading the page by calling
 * the load() or reload() method. The loading can also be triggered by the
 * management methods of the navigation history, discussed
 * @ref browser-feature-history "below". When the load is complete, the
 * controller emits the loadFinished() signal.
 *
 ...
 * anchor inside the current page. The controller must not load a page or create
 * a new one itself when activating a link - the application does this in
 * response to the linkClicked() signal. The controller must not create and show
 ...
 *
 * For integration into the GUI, the view() method returns a widget with the page
 * displayed. The controller manages the lifecycle of its child objects QWidget
 * and, if any, Browser::History.
 *
 *
 ...
 * Note that for a supported feature, the controller must have a complete
 ...
 *
 * A controller that supports the history must meet the following conditions:
 *
 * * The hasFeature() method called with the Browser::FEATURE_HISTORY must
 *   return @c true.
 ...
 *
 * A controller that supports text copying must meet the following conditions:
 *
 * * The hasFeature() method called with the Browser::FEATURE_COPY must
 *   return @c true.
 ...
 *
 * A controller that supports text searching must meet the following conditions:
 *
 * * The hasFeature() method called with the Browser::FEATURE_SEARCH must
 *   return @c true.
 ...
 * <h3>Page printing</h3>
 *
 * A controller that supports page printing must meet the following conditions:
 *
 * * The hasFeature() method called with the Browser::FEATURE_PRINT must
 *   return @c true.
 ...
 * <h3>Page zooming</h3>
 *
 * A controller that supports page zooming must meet the following conditions:
 *
 * * The hasFeature() method called with the Browser::FEATURE_ZOOM must
 *   return @c true.
 * * The zoomFactor(), setZoomFactor(), zoomDecrease() and zoomIncrease()
 *   methods must be fully implemented.
 *
 *
 * @anchor browser-feature-scroll
 * <h3>Page scrolling</h3>
 *
 * A controller that supports page scrolling must meet the following conditions:
 *
...
class Controller : public QObject
{
	...
	public:
		 ...
		 */
		explicit Controller(ContentProvider::Ptr contentProvider, QObject* parent);
	...
	public:
		 ...
		 * @see configure()
		 */
		virtual bool hasOption(Browser::Option option);

		/**
		 * @brief Configures the controller.
		 *
		 * The controller may not support some of the options available in the
		 * BrowserSettings structure. Use the hasOption() method to check for
		 * supported options.
		 *
		 * @param[in] settings The settings that apply.
		 */
		virtual void configure(const Browser::Settings& settings);
		 ...
		 */
		virtual void load(const QUrl& address);
		 ...
		 */
		virtual bool hasFeature(Browser::Feature feature) const;

		/**
		 * @brief Returns a pointer to the history of navigated web pages.
		 *
		 * If controller has not Browser::FEATURE_HISTORY feature, the method
		 * return @c nullptr.
		 *
		 * @return Returns a pointer to the Browser::History object or @c nullptr.
		 */
		virtual Browser::History* history() const;
	...
	public:
		...
		/// @see setZoomFactor()
		static constexpr qreal ZOOM_MIN = 0.2;
		/// @brief Maximum scaling factor.
		/// @see setZoomFactor()
		static constexpr qreal ZOOM_MAX = 5.0;

		/**
		 * @brief Returns the page scaling.
		 *
		 * If controller has not Browser::FEATURE_ZOOM feature, the method
		 * must returns 1.0.
		 *
		 * @see setZoomFactor()
		 * @see zoomDecrease()
		 ...
		 * @see zoomFactor()
		 */
		virtual void setZoomFactor(qreal zoom);
	...
	public:
		 ...
		 * @see scrollTop()
		 */
		virtual void setScrollTop(int pos);
		 ...
		 * @param[in] pos The position of the scroll bar.
		 */
		virtual void setAutoScroll(int pos);
		 ...
		 * @see selectAll()
		 */
		virtual void selectedCopy();

		/**
		 * @brief The FindFlags structure contains flags for the findText() method.
		 */
		struct FindFlags {
		 ...
		 *
		 * If controller has not Browser::FEATURE_SEARCH feature, the method
		 * does nothing.
		 *
		 ...
		 *
		 * If controller has not Browser::FEATURE_PRINT feature, the method
		 * does nothing.
		 *
		 * @param[in] printer Print device.
		 */
		virtual void print(QPrinter* printer, std::function<void (bool succes)> result);
	...
	signals:
		 ...
		 */
		void loadFinished(Controller* self, bool ok);
		 ...
		 */
		void urlChanged(Controller* self, const QUrl& newUrl);
		 ...
		 */
		void contextMenuRequested(Controller* self, const QPoint& pos, const QUrl& link);
		 ...
		 *     a new one.
		 */
		void linkClicked(Controller* self, const QUrl& link, Browser::OpenFlag flag);
		 ...
		 * @param[in] self Pointer to the controller that emits the signal.
		 */
		void historyChanged(Controller* self);
		 ...
		 */
		void findTextFinished(Controller* self, bool hit, int count, int pos);
		 ...
		 */
		void printFinished(Controller* self, bool ok);
};
