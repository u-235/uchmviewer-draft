...
void UBrowser::HistoryImpl::onUrlChanged( const QUrl& url )
{
	...
	if ( !currentItem().url.matches( url,  QUrl::None ) )
	{
		...
		QString title = m_browser->contentProvider()->getTopicByUrl( url );

		if ( title.isEmpty() )
			title = m_browser->title().trimmed();

		m_items[m_currentItemIndex].url = url;
		m_items[m_currentItemIndex].scroll = 0;
		m_items[m_currentItemIndex].title = title;
	}

	emit historyChanged();
}

void UBrowser::HistoryImpl::setScrollFragment( int scroll )
{
#ifdef PRINT_DEBUG
	qDebug() << "HistoryImpl.setScrollFragment";
	qDebug() << "    scroll =" << scroll;
#endif

	m_scrollFragment = scroll;
	m_items[m_currentItemIndex].scroll = -1;
}

void UBrowser::HistoryImpl::setScroll( int scroll )
{
#ifdef PRINT_DEBUG
	qDebug() << "HistoryImpl.setScroll";
	qDebug() << "    scroll =" << scroll;
#endif

	if ( scroll == m_scrollFragment )
	{
#ifdef PRINT_DEBUG
		qDebug() << "    The page did not scroll, -1 was recorded.";
#endif
	...
	{
#ifdef PRINT_DEBUG
		qDebug() << "    The page was scrolled and the actual value was recorded.";
#endif
