 ...
 * * load()
 * * reload()
 * * All methods of UBrowser::Feature::history feature:
 *   * history()
 *   * canGoBack()
 *   * canGoForward()
 *   * back()
...
class UBROWSER_API_DECL AbstractBrowser : public UBrowser::Browser
{
	...
	protected:
		explicit AbstractBrowser( ContentProvider::Ptr contentProvider, QObject* parent );
	...
	public slots:
		 ...
		 *
		 * @param[in] globalPos  Global position of requested menu.
		 * @param[in] link
		 */
	...
	private:
		bool m_pageLoads;
		bool m_pageReveal;
		// The page reveal signal was received before the loading was complete.
		bool m_prematurePageReveal;
		// The URL change signal was received before the loading was complete.
		bool m_prematureUrlChanged;
