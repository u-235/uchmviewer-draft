...
#define CONTENT_KIND_HTML "html"
#define CONTENT_KIND_RTF  "rtf"
#define CONTENT_KIND_TXT  "txt"
...
class ContentProvider
{
	public:
		...
		*/
		...
		/**
		 * @brief Returns the ContentProvider kind.
		 *
		 * Currently there is only the "html" kind; there may be "txt" and
		 * "rtf" types.
		 *
		 * @return A string with the ContentProvider kind.
		 */
		 ...
		 *     stored.
		 * @param[in]  url  An URL in eBook to retrieve content from.
		 * @return Returns @c true if operation is success.
		 *
		 * @see MimeHelper
		 */
		virtual bool content( ContentData& data, const QUrl& url ) = 0;

		/**
		 * @brief Gets the Title of the page referenced by url.
		 *
		 * @param[in] url An URL in ebook file to get title from. Must be absolute.
		 * @return The title, or empty string if the URL cannot be found or not a HTML page.
		 */
