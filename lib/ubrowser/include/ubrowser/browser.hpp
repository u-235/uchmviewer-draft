...
#define UBROWSER_KIND_HTML CONTENT_KIND_HTML
#define UBROWSER_KIND_RTF  CONTENT_KIND_RTF
#define UBROWSER_KIND_TXT  CONTENT_KIND_TXT
 ...
 * settings options via the UBrowser::Builder::configure() method, and the
 * hasOption() method should inform the application of supported options.
 *
 ...
 *
 * * The hasFeature() method called with the UBrowser::Feature::history must
 *   return @c true.
 ...
 *
 * * The hasFeature() method called with the UBrowser::Feature::copy must
 *   return @c true.
 ...
 *
 * * The hasFeature() method called with the UBrowser::Feature::search must
 *   return @c true.
 ...
 *
 * * The hasFeature() method called with the UBrowser::Feature::print must
 *   return @c true.
 ...
 *
 * * The hasFeature() method called with the UBrowser::Feature::zoom must
 *   return @c true.
 * * The zoomFactor() and realSetZoomFactor() methods must be fully implemented.
 *
 *
 ...
 *
 * * The hasFeature() method called with the UBrowser::Feature::scroll must
 *   return @c true.
...
class UBROWSER_API_DECL Browser : public QObject
{
	...
	public:
		 ...
		 *
		 * @param[in] contentProvider The content provider.
		...
		/**
		 * @brief  Returns the content provider.
		 */
		ContentProvider::Ptr contentProvider() const;

		/**
		 * @brief Sets the content provider.
		 */
		 ...
		 *
		 * If browser has not UBrowser::Feature::history feature, the method
		 * return @c nullptr.
		 *
	...
	public:
		 ...
		 *
		 * If browser has not UBrowser::Feature::zoom feature, the method
		 * must returns 1.0.
		 *
		 * @see setZoomFactor()
		 * @see zoomDecrease()
		 ...
		 *
		 * The method must limit the zoom factor to a range from zoomFactorMin()
		 * to zoomFactorMax() inclusive.
		 *
		 * If browser has not UBrowser::Feature::zoom feature, the method
		 * does nothing.
		 *
		...
		/**
		 * @brief Returns the minimum zoom limit.
		 */
		qreal zoomFactorMin();

		/**
		 * @brief Sets the minimum zoom limit.
		 */
		void setZoomFactorMin( qreal min );

		/**
		 * @brief Returns the maximum zoom limit.
		 */
		qreal zoomFactorMax();

		/**
		 * @brief Sets the maximum zoom limit.
		 */
		void setZoomFactorMax( qreal max );

		/**
		 * @brief Returns the zoom step.
		 */
		qreal zoomFactorStep();

		/**
		 * @brief Sets the zoom step.
		 *
		 * @param step The zoom step.
		 * @see zoomIncrease()
		 * @see zoomDecrease()
		 */
		void setZoomFactorStep( qreal step );

	public slots:
		/**
		 * @brief Zooms in on the page.
		 *
		 * The method zooms in by the zoomFactorStep() value as long as the zoom
		 * is not larger than zoomFactorMax().
		 *
		 * If browser has not UBrowser::Feature::zoom feature, the method
		 * does nothing.
		 * @see zoomDecrease()
		 ...
		 *
		 * The method zooms out by the zoomFactorStep() value as long as the zoom
		 * is not smaller than zoomFactorMin().
		 *
		 * If browser has not UBrowser::Feature::zoom feature, the method
		 * does nothing.
		 * @see zoomIncrease()
	...
	public:
		 ...
		 *
		 * If browser has not UBrowser::Feature::scroll feature, the method
		 * must returns @c 0.
		 *
		...
		/**
		 * @brief Sets the scroll position that will be applied after the page
		 * loads.
		 *
		 ...
		 *
		 * If browser has not UBrowser::Feature::copy feature, the method
		 * does nothing.
		 *
		 * @see selectedText()
		 * @see selectedCopy()
		 ...
		 *
		 * If browser has not UBrowser::Feature::search feature, the method
		 * does nothing.
		 *
		 * @param[in] text The text to be found.
		 * @param[in] backward Search from the end of the page to the head of
		 *            the page.
		 * @param[in] caseSensitively When searching, take the case into account.
		 * @param[in] highlightSearchResults Highlight hits with the search pattern.
		 * @param[in] result
		 */
		 ...
		 *
		 * If browser has not UBrowser::Feature::print feature, the method
		 * does nothing.
		 *
		...
		virtual void print( QPrinter* printer,
		                    std::function<void ( bool success )> result ) = 0;

	protected:
		int autoScroll();
		void scrollAfterLoad();
		virtual void realSetZoomFactor( qreal zoom ) = 0;
	...
	signals:
		 ...
		 * UBrowser::Feature::history feature.
		 */
		void historyChanged();
		 ...
		 *
		 * The signal works if the browser supports the
		 * UBrowser::Feature::search feature.
		 *
		 ...
		 *
		 * The signal works if the browser supports the
		 * UBrowser::Feature::print feature.
		 *
